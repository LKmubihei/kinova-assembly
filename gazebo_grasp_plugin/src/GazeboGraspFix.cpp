...
void GazeboGraspFix::OnUpdate()
{
       ...
       ocIt != objectContactInfo.end(); ++ocIt)
  {
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("grasp"), "Grasp Held: "<<objName<<" grip count: "<<this->gripCounts[objName]);
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("grasp"), "GRIPPING "<<objName<<", grip count "<<counts<<" (threshold "<<this->gripCountThreshold<<")");
    ...
    if (isAttachedToGripper)
    {
      ...
      RCLCPP_DEBUG_STREAM(rclcpp::get_logger("grasp"), "GazeboGraspFix has found that object that is already attached");
