...
# bin_pcb_detection 根目录 (scripts的上一级)
BIN_ROOT = SCRIPT_DIR.parent

# 数据集配置文件 - 直接使用绝对路径（确保能找到）
DATA_YAML = Path("/home/pb/Hisense_pcb_inspection/src/Hisense_assemble-main/bin_pcb_detection/data.yaml")
...
def train_detect():
    ...
    # 保存最佳模型到models目录
    best_model = RESULTS_DIR / "bin_pcb_detect" / "weights" / "best.pt"
