...
def visualmodule(camera_data, camera_Transfor_in_base, model, phase):
        ...
        if len(boxes) > 0:
            for box, box_xyxy, cls_ in zip(boxes, boxes_xyxy, classes):
                if int(cls_) == 0:      ## redgear
                    ...
                    depth_value = depth_image[circle_y, circle_x]/camera_data.depth_scale                    
                    # Calculate the object's position in the base frame
                    pixel_cam = pixel_to_camera(circle_x, circle_y, camera_data.K_depth, depth_value)
                    target_to_base = camera_Transfor_in_base @ pixel_cam                    
                    print(f"object center coordinates: {circle_x}, {circle_y}")
                    print(f"Depth value: {depth_value} m")
                    print(f"camera ini param: {camera_data.K_depth}")
...
def process_target_in_base(tf_to_depth_camera, camera_data, model, phase):
    """
    处理并获取目标相对于基坐标系的位置。
    ...
    while rclpy.ok():
        ...
        # 处理相机数据，获取目标变换
        target_to_base = visualmodule(camera_data, camera_T_in_base, model, phase)
...
def main(args=None):
    ...
    camera_data = GGCNNService(visual_node, "/camera")
    ...
    target_offset_y = 0.010
    target_offset_z = 0.004
    gripper_open= Gripper_cmd(1000)
    gripper_grab = Gripper_cmd(800)
    model = YOLO('/home/robotck/ur_ros2_driver/src/Universal_Robots_ROS2_Driver/ur_bringup/scripts/best.pt')
    move_cmd_initial = Move_cmd(initial_grasp_P_in_base[0], initial_grasp_P_in_base[1], initial_grasp_P_in_base[2], initial_grasp_Euler_in_base[0], initial_grasp_Euler_in_base[1], initial_grasp_Euler_in_base[2], vel, acc)


    try:
        gripper_cmd_node.send_gripper_command(gripper_open)
        target_to_base = process_target_in_base(tf_to_depth_camera, camera_data, model, 1)
        move_cmd = Move_cmd(target_to_base[0] - camera_to_grasp_in_base[0], target_to_base[1] - camera_to_grasp_in_base[1], initial_grasp_P_in_base[2], initial_grasp_Euler_in_base[0], initial_grasp_Euler_in_base[1], initial_grasp_Euler_in_base[2], vel, acc)
        robot_move_node.send_goal(move_cmd)

        target_to_base = process_target_in_base(tf_to_depth_camera, camera_data, model, 2)
        move_cmd = Move_cmd(target_to_base[0]+target_offset_x, target_to_base[1]+target_offset_y, target_to_base[2] + target_offset_z, initial_grasp_Euler_in_base[0], initial_grasp_Euler_in_base[1], initial_grasp_Euler_in_base[2], vel, acc)
        robot_move_node.send_goal(move_cmd)
        time.sleep(1)
        ...
        target_to_base = process_target_in_base(tf_to_depth_camera, camera_data, model, 3)
        move_cmd = Move_cmd(target_to_base[0] - camera_to_grasp_in_base[0], target_to_base[1] - camera_to_grasp_in_base[1], initial_grasp_P_in_base[2], initial_grasp_Euler_in_base[0], initial_grasp_Euler_in_base[1], initial_grasp_Euler_in_base[2], vel, acc)
        robot_move_node.send_goal(move_cmd)

        target_to_base = process_target_in_base(tf_to_depth_camera, camera_data, model, 4)
        move_cmd = Move_cmd(target_to_base[0]+target_offset_x, target_to_base[1]+target_offset_y, target_to_base[2] + target_offset_z + 0.005, initial_grasp_Euler_in_base[0], initial_grasp_Euler_in_base[1], initial_grasp_Euler_in_base[2], vel, acc)
        robot_move_node.send_goal(move_cmd)
        time.sleep(1)
        gripper_cmd_node.send_gripper_command(gripper_open)
        time.sleep(1)
