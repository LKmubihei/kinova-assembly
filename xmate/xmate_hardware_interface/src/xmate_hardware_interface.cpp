#include "xmate_hardware_interface/xmate_hardware_interface.hpp"
...
XmateHardwareInterface::XmateHardwareInterface() :
_sim_flag(false), 
mode(e_mode::in_Position),
logger_(rclcpp::get_logger("XmateHardwareDriver")),
node_(std::make_shared<rclcpp_lifecycle::LifecycleNode>("XmateHardwareDriver"))
{}

XmateHardwareInterface::~XmateHardwareInterface(){}

e_mode XmateHardwareInterface::convertToEMode(signed char value) {
    switch (value) {
...
hardware_interface::CallbackReturn XmateHardwareInterface::on_init(const hardware_interface::HardwareInfo & info){
    ...
    */
    ...
    if (robot._dof == 7){
        RCLCPP_INFO(logger_, "robot joint num = %zu.", robot._dof);
    }else{
        RCLCPP_ERROR(logger_, "robot joint num error ,num = %zu.", robot._dof);
        return hardware_interface::CallbackReturn::ERROR;
    }
    ...
    */
    ...
    if (robot._dof != robot.joints_num){
        RCLCPP_ERROR(logger_, "robot device dof != joint num ! -- device dof: %zu -- joint num: %zu ",robot._dof, robot.joints_num);
        return hardware_interface::CallbackReturn::ERROR;
    }
    ...
    */
    ...
    if (it == info_.hardware_parameters.end()){
        RCLCPP_ERROR(logger_, "Ros2 can't find parammeter \"sim_flag\"");
        return hardware_interface::CallbackReturn::ERROR;
    }else{
        std::string tem = "True";
    ...
    */
    ...
    if (!_sim_flag){
        if (it == info_.hardware_parameters.end()){
            RCLCPP_ERROR(logger_, "Ros2 can't find parammeter \"device_mode\"");
            return hardware_interface::CallbackReturn::ERROR;
        }else{
...
std::vector<hardware_interface::StateInterface> XmateHardwareInterface::export_state_interfaces(){
...
std::vector<hardware_interface::CommandInterface> XmateHardwareInterface::export_command_interfaces(){
...
hardware_interface::CallbackReturn XmateHardwareInterface::on_configure(const rclcpp_lifecycle::State & previous_state){    

    return CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn XmateHardwareInterface::on_activate(const rclcpp_lifecycle::State & previous_state){
...
hardware_interface::CallbackReturn XmateHardwareInterface::on_deactivate(const rclcpp_lifecycle::State & previous_state){
...
hardware_interface::return_type XmateHardwareInterface::read(const rclcpp::Time & time, const rclcpp::Duration & period){
...
hardware_interface::return_type XmateHardwareInterface::write(const rclcpp::Time & time, const rclcpp::Duration & period){
...
PLUGINLIB_EXPORT_CLASS(xmate_hardware_interface::XmateHardwareInterface, hardware_interface::SystemInterface)
