...
def get_flange_pose(tf_buffer, tf_listener_node, from_frame='tool0', to_frame='ur_real_base'):

    # Retrieve the transformation
    flange_in_base = tf_transformation(tf_buffer, from_frame, to_frame, tf_listener_node)

    if flange_in_base is None:
        tf_listener_node.get_logger().info('Failed to retrieve transformation')
        return None

    # Extract translation vector
    flange_P_in_base = np.array([
        flange_in_base.transform.translation.x,
        flange_in_base.transform.translation.y,
        flange_in_base.transform.translation.z
    ])

    # Extract rotation as a matrix
    flange_R_in_base = R.from_quat([
        flange_in_base.transform.rotation.x,
        flange_in_base.transform.rotation.y,
        flange_in_base.transform.rotation.z,
        flange_in_base.transform.rotation.w
    ]).as_matrix()
