...
private:
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
        ...
        // 调整速度缩放因子
        // scale_trajectory_speed(trajectory, SPEED);
    ...
    void scale_trajectory_speed(moveit_msgs::msg::RobotTrajectory &trajectory, double speed_scale)
    {
        if (speed_scale <= 0.0) {
            RCLCPP_ERROR(this->get_logger(), "无效的 speed_scale: %f。速度缩放因子必须大于零。", speed_scale);
            return;
        }
        ...
        for (auto & point : trajectory.joint_trajectory.points) {
            ...
            // 将时间转换为纳秒
            int64_t total_nanoseconds = static_cast<int64_t>(time_from_start.sec) * 1'000'000'000LL + time_from_start.nanosec;

            // 缩放时间
            int64_t scaled_nanoseconds = static_cast<int64_t>(total_nanoseconds * (1.0 / speed_scale));

            // 更新 sec 和 nanosec
            time_from_start.sec = static_cast<int32_t>(scaled_nanoseconds / 1'000'000'000LL);
            time_from_start.nanosec = static_cast<uint32_t>(scaled_nanoseconds % 1'000'000'000LL);
        }
    }
